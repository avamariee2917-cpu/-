...
// Commands that must always be included in Discord registration.
...
export async function loadCommands(client) {
    ...
    if (client.commands.has('br')) {
        logger.info(
            '★ Booster command /br successfully loaded.'
        );
    } else {
        logger.warn(
            '⚠ Booster command /br was NOT loaded.'
        );
    }

    if (client.commands.has('boosters')) {
        logger.info(
            '★ Booster command /boosters successfully loaded.'
        );
    } else {
        logger.warn(
            '⚠ Booster command /boosters was NOT loaded.'
        );
    }
...
function prepareCommandsForRegistration(commands) {
    ...
    logger.warn(
        `Command count (${commands.length}) exceeds Discord limit (${MAX_COMMANDS}), prioritizing required booster commands...`
    );

    const priorityCommands =
        commands.filter(
            command =>
                PRIORITY_COMMANDS.has(
                    command.name
                )
        );
    ...
    logger.info(
        `Prioritized commands registered: ${priorityCommands.map(command => command.name).join(', ') || 'none'}`
    );

    logger.info(
        `Registered ${truncated.length} commands total`
    );
    ...
    if (
        ...
        logger.warn(
            `Skipped ${skippedCommands.length} commands because Discord allows a maximum of ${MAX_COMMANDS} top-level commands.`
        );
    }
...
async function registerGuildCommands(
    ...
    if (!guildId) {
        throw new Error(
            'GUILD_ID is required for server-specific slash command registration'
        );
    }
    ...
    logger.info(
        `Preparing to register ${totalSubcommands + commands.length} commands to guild ${guildId}`
    );

    logger.info(
        'Validating commands before guild registration...'
    );
    ...
    logger.info(
        'Guild command validation passed'
    );
    ...
    const registeredBoosterCommands =
        commandsToRegister
            .filter(
                command =>
                    PRIORITY_COMMANDS.has(
    ...
    logger.info(
        `Booster commands registered: ${registeredBoosterCommands.join(', ') || 'NONE'}`
    );
}
